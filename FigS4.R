...
for(i in 1:4){ # going through each subset...
  ...
  for(j in 1:4){ # now going through each spatial scale at the subset...
    ...
    # compute observed NNIs - FOR SUPPLEMENT FIGURE
    obs.NNI <- (obs.x$mean_NN - sta.x$null_meanNN_mean) / sta.x$null_meanNN_sd
    obs.NNI <- na.omit(obs.NNI)
    
    # compare observed NNIs to null NNIs
    obs.NNI.mean <- mean(obs.NNI, na.rm = T)
    p_hi <- sum(dis.x$null_NNI_mean > obs.NNI.mean) / 1000
    p_lo <- sum(dis.x$null_NNI_mean < obs.NNI.mean) / 1000
    
    
    plot.dat[[j]] <- c(obs.NNI.mean, stars.pval(p_lo), stars.pval(p_hi))
    
  }
  
  # back to our per subset situation... lets prepare plotting data.
  box.dat <- cbind(dis[[1]]$null_NNI_mean, dis[[2]]$null_NNI_mean, dis[[3]]$null_NNI_mean, dis[[4]]$null_NNI_mean)
  ...
  colnames(plot.dat) <- c("obs.NNI", "pval.lo", "pval.hi")
  plot.dat$scale <- c("10m", "20m", "50m", "100m")
  plot.dat$obs.NNI <- as.numeric(plot.dat$obs.NNI)
  plot.dat <- plot.dat %>%
    mutate(scale = fct_relevel(scale, "10m", "20m", "50m", "100m"))
